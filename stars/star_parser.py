...
def parse_stars(asked_string):
    ...
    while ptr < len(asked_string):
        mat_pair = match_one_star(asked_string, ptr)
        if mat_pair is None:
...
def match_one_star(asked_string, ptr=0):
    mat_pair = None
    for filt in filters:
        regex: re.Pattern
...
def parse_one_star(asked_string):
    mat_pair = match_one_star(asked_string)
    if mat_pair is None:
        return None
    mat, proc = mat_pair
    if proc is None:
